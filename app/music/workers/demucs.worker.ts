...
async function createInstrumentalMix(sepDir: string): Promise<string | null> {
  ...
  for (const s of ["drums", "bass", "guitar", "piano", "other"]) {
    ...
    if (fs.existsSync(mp3)) stems.push(mp3);
    else if (fs.existsSync(wav)) stems.push(wav);
  }
...
export const demucsWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    if (!stemUrls.vocalsUrl) throw new Error("No stems found");
...
demucsWorker.on("failed", (job, err) =>
  console.error("[demucs-oracle] ✗", job?.id, err.message)
);
