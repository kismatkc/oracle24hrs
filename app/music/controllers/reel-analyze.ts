...
const BASE_ARGS: readonly string[] = ["--proxy", PROXY_URL, "--user-agent", REEL_UA] as const;
...
const PIPELINE_TIMEOUT = 110_000; // 110s — slightly under nginx 120s proxy timeout
...
function getGroq2(): Groq {
  ...
  if (!key) throw new Error("GROQ_API_KEY_2 not configured — add it to Doppler");
  return new Groq({ apiKey: key });
}

// ─── URL Validation ───────────────────────────────────────────────────────────

function validateUrl(url: unknown): { valid: true; url: string } | { valid: false; error: string } {
  if (!url || typeof url !== "string") return { valid: false, error: "url is required" };
  if (!/^https?:\/\//i.test(url)) return { valid: false, error: "url must be HTTP/HTTPS" };
  if (!/instagram\.com/i.test(url)) return { valid: false, error: "url must be an Instagram URL" };
...
function downloadAudio(reelUrl: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (code !== 0) return reject(new Error(`yt-dlp audio failed (${code}): ${stderr.slice(-200)}`));
      if (!fs.existsSync(tempFile)) return reject(new Error("audio file not found after download"));
      resolve(tempFile);
    });
  });
}
...
async function runPipeline1(reelUrl: string): Promise<string> {
...
function downloadVideo(reelUrl: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", reject);
    proc.on("close", (code) => {
      if (code !== 0) return reject(new Error(`yt-dlp video failed (${code}): ${stderr.slice(-200)}`));
      if (!fs.existsSync(tempFile)) return reject(new Error("video file not found after download"));
      resolve(tempFile);
    });
  });
}

function extractFrames(videoPath: string, framesDir: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("error", reject);
    proc.on("close", (code) => {
      if (code !== 0) return reject(new Error(`ffmpeg failed (${code}): ${stderr.slice(-200)}`));
      try {
...
async function extractTextFromFrame(imagePath: string, groq2: Groq): Promise<string> {
...
function deduplicateConsecutive(texts: string[]): string {
  const kept: string[] = [];
  let prevNorm = "";
  ...
  for (const text of texts) {
    ...
    if (overlap && prevNorm.length > 10) continue;
    kept.push(t);
    prevNorm = norm;
  }
...
async function runPipeline2(reelUrl: string): Promise<string> {
  ...
  try {
    ...
    if (!result) throw new Error("No on-screen text detected in this reel");
    return result;
  } finally {
...
async function analyzeReel(req: Request, res: Response): Promise<void> {
