...
function listFilesInDir(dirPath: string): {
  ...
  for (const ent of fs.readdirSync(dirPath, { withFileTypes: true })) {
    ...
    if (size === 0 && !ent.isDirectory()) continue;
    totalSize += size;
...
// ─── GET /cache/youtube — list cached YouTube audio files ────────────────────

interface CachedYoutubeItem {
  videoId: string;
  title: string;
  author: string;
  duration: number;
...
router.get("/cache/youtube", async (_req: Request, res: Response) => {
  try {
...
router.delete("/cache/youtube", async (_req: Request, res: Response) => {
  try {
    if (!fs.existsSync(YOUTUBE_CACHE_DIR)) {
    ...
    console.log(
      `[cache-mgmt] Cleared all youtube cache: ${deletedCount} files`
    );
...
// ─── GET /cache/stems — list separated stem directories ──────────────────────

interface CachedStemItem {
  songId: string;
  dirSize: number;
  dirSizeFormatted: string;
  stemCount: number;
  stems: string[];
  createdAt: number;
  ageHours: number;
...
router.get("/cache/stems", async (_req: Request, res: Response) => {
  try {
...
router.delete("/cache/stems/:songId", async (req: Request, res: Response) => {
  try {
...
router.delete("/cache/stems", async (_req: Request, res: Response) => {
  try {
    const stemsRoot = path.join(SEPARATED_DIR, model);
    if (!fs.existsSync(stemsRoot)) {
      res.json({ success: true, deletedCount: 0 });
...
router.get("/cache/uploads", async (_req: Request, res: Response) => {
  try {
...
router.delete("/cache/uploads/:name", async (req: Request, res: Response) => {
  try {
...
router.delete("/cache/uploads", async (_req: Request, res: Response) => {
  try {
...
router.get("/cache/normalized", async (_req: Request, res: Response) => {
  try {
...
router.delete("/cache/normalized", async (_req: Request, res: Response) => {
  try {
...
router.delete("/cache/all", async (_req: Request, res: Response) => {
  try {
    let totalDeleted = 0;
    ...
    console.log(
      `[cache-mgmt] ☢️ NUKED all Oracle cache: ${totalDeleted} items`
    );
