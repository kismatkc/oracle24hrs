...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    console.log(
      `[shazam] Received ${file.originalname} (${(file.size / 1024).toFixed(1)} KB) → ${filePath}`
    );

    try {
      const result = await runPython(filePath);
      console.log(
        `[shazam] ${result.success ? "Match" : "No match"}: ${result.title || result.error || "—"}`
      );
...
function runPython(audioPath: string): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    proc.on("close", (code) => {
      if (stderr) console.warn("[shazam/py stderr]", stderr.trim());
      ...
      try {
        ...
        reject(
          new Error(
            `Python exited ${code}, parse error: ${e.message}\nstdout: ${stdout}\nstderr: ${stderr}`
          )
        );
      }
    });
