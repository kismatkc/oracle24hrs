...
async function upsertOptionalInfoH(
  activity: string,
  label: string,
  date: string
) {
...
async function getOptionalInfoH(activity: string) {
  const h = await Redis.hgetall(streakKey(activity));
  if (!h || (!h.info_label && !h.info_date)) return null;
  if (!h.info_label || !h.info_date) return null;
  return { label: h.info_label, date: h.info_date };
}
async function clearOptionalInfoH(activity: string) {
  const exists = await Redis.exists(streakKey(activity));
  if (!exists) return false;
...
async function streaks(req: Request, res: Response) {
  try {
    ...
    if (event === "addSuccessfulDate") {
      ...
      const { activity, label, date } = req.body as any;
