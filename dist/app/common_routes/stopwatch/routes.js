...
import { loadDay, createDay, saveDay, appendOpenSegment, closeOpenSegment, getSegments, sumSegments, shrinkLastSegment, ensureRepeatJob, clearRepeatJob, clearAllSegments, ensureGoalJob, clearGoalJob, 
// NEW criterion-aware helpers
appendOpenCriterionSegment, closeOpenCriterionSegment, getCriterionSegments, shrinkLastCriterionSegment, ensureCriterionGoalJob, clearCriterionGoalJob, overallProgressMs, allCriteriaMet, } from "./queue.js";
...
function normalizeDayKey(input) {
    ...
    if (!input)
        return today;
    try {
        const deltaDays = (parseYYYYMMDD(input) - parseYYYYMMDD(today)) / 86400000;
        if (deltaDays === 1)
            return today; // coerce common +1d UTC bug
    }
    catch { }
    return input;
}
/* ───────────── Habits storage ───────────── */
const HABITS_SET = (userId) => `stopwatch:${userId}:habits`;
const HABIT_HASH = (userId, id) => `stopwatch:${userId}:habit:${id}`;
const SELECTED_KEY = (userId) => `stopwatch:${userId}:habit:selected`;
const TITLE_INDEX = (userId) => `stopwatch:${userId}:titleIndex`;
...
async function listHabits(userId) {
    ...
    if (!ids?.length)
        return [];
    const pipeline = ids.map((id) => ["hgetall", HABIT_HASH(userId, id)]);
...
class DuplicateTitleError extends Error {
    constructor() {
        super(...arguments);
        this.code = "DUPLICATE_TITLE";
    }
}
async function upsertHabit(userId, input) {
    ...
    const mappedId = (await Redis.hget(TITLE_INDEX(userId), newNorm));
    ...
    const toSet = { title, minutes: String(minutes) };
    if (criteria)
        toSet.criteria = JSON.stringify(criteria);
    await Redis.hset(HABIT_HASH(userId, id), toSet);
    await Redis.sadd(HABITS_SET(userId), id);
    // update title index
    if (prevNorm && prevNorm !== newNorm)
        await Redis.hdel(TITLE_INDEX(userId), prevNorm);
    ...
    if (dayKeys?.length) {
        ...
        for (const dayKey of dayKeys) {
            ...
            for (const c of crits)
                await clearCriterionGoalJob(id, dayKey, c.id);
        ...
        await Redis.multi(multi).exec();
    }
    await Redis.del(daysIndex);
}
async function getSelected(userId) {
    return await Redis.get(SELECTED_KEY(userId));
}
async function setSelected(userId, habitId) {
    await Redis.set(SELECTED_KEY(userId), habitId);
}
/* ---------- TODAY filter (lenient) ---------- */
async function filterTodayRelevant(userId, habits) {
    if (!habits.length)
        return habits;
    const today = serverLocalDayKey();
    const multi = [];
    ...
    const rows = await Redis.multi(multi).exec();
    const keep = new Set();
    for (let i = 0; i < habits.length; i++) {
        const stHash = rows[2 * i]?.[1];
        const segLen = Number(rows[2 * i + 1]?.[1] ?? 0);
        const status = stHash?.status;
        const hasState = !!status;
        const show = segLen > 0 || !hasState || status !== "canceled";
        if (show)
            keep.add(habits[i].id);
    }
    return habits.filter((h) => keep.has(h.id));
}
/* ───────────── Habits routes ───────────── */
router.get("/stopwatch/habits", async (req, res) => {
    try {
        ...
        const bypass = String(req.query.all || "") === "1";
...
router.post("/stopwatch/habits", async (req, res) => {
    try {
        ...
        if (!sel)
            await setSelected(userId, habit.id);
...
router.delete("/stopwatch/habits/:id", async (req, res) => {
    try {
        const { userId } = req.query;
        const { id } = req.params;
        if (!userId || !id) {
            res.status(400).json({ error: "userId and id required" });
            return;
        }
        await deleteHabit(userId, id);
        const items = await listHabits(userId);
        if (items.length && !(await getSelected(userId)))
            await setSelected(userId, items[0].id);
...
router.get("/stopwatch/habits/selected", async (req, res) => {
    try {
        const { userId } = req.query;
        if (!userId) {
            res.status(400).json({ error: "userId required" });
...
router.post("/stopwatch/habits/selected", async (req, res) => {
    try {
        const { userId, habitId } = req.body || {};
        if (!userId || !habitId) {
            res.status(400).json({ error: "userId and habitId required" });
            return;
        }
...
async function getTimerState(req, res) {
    try {
        ...
        const minDailyMs = Number(req.query.minDailyMs ?? 0);
        const criterionId = req.query.criterionId;
        let dayKey = normalizeDayKey(req.query.dayKey);
        if (!habitId || !userId || !dayKey) {
            res.status(400).json({ error: "habitId,userId,dayKey required" });
            return;
        }
        let day = await loadDay(habitId, dayKey);
        if (!day)
            day = await createDay({ habitId, userId, dayKey, minDailyMs });
...
async function confirmTimer(req, res) {
    try {
...
async function adjustTimer(req, res) {
    try {
