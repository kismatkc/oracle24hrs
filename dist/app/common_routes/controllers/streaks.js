...
async function readItems(activity) {
    const h = await Redis.hgetall(streakKey(activity));
    if (!h)
        return [];
    // New store
    const items = h.info_items ? JSON.parse(h.info_items) : [];
...
async function writeItems(activity, items) {
...
async function upsertOptionalInfoH(activity, label, date) {
    ...
    const item = { id: nowId(), label, date: date ?? null };
    items.push(item);
...
async function clearOptionalInfoH(activity, id) {
    const exists = await Redis.exists(streakKey(activity));
    if (!exists)
        return false;
    if (!id) {
...
async function streaks(req, res) {
    try {
        ...
        if (event === "addSuccessfulDate") {
            ...
            if (exists) {
                res.status(409).json({ success: false, msg: "A habit with that title already exists" });
                return;
            }
            ...
            const { activity, label } = req.body;
            const date = req.body.date ?? null; // optional
            if (!activity || !label) {
                res.status(400).json({ success: false, msg: "activity and label are required" });
                return;
            }
            ...
            res.status(201).json({ success: true, data: r.item });
            return;
        }
        else if (event === "clearOptionalInfo") {
            const { activity, id } = req.body; // id optional → clear all
            if (!activity) {
