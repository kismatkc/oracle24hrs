...
const REEL_ARGS_BASE = [
    "--proxy",
    PROXY_URL,
    "--user-agent",
    "Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/17.0 Mobile/15E148 Safari/604.1",
];
const HARD_TIMEOUT_MS = 120000; // 2 min — matches nginx proxy_read_timeout 120s
...
function validateReelUrl(url) {
    ...
    if (!shortcode) {
        return {
            valid: false,
            error: "Could not extract reel shortcode from URL. Supported formats: /reel/CODE/, /p/CODE/, /tv/CODE/",
        };
    }
...
function getReelDuration(reelUrl) {
...
function downloadReelAudio(reelUrl) {
...
// ─── Main Route Handler ───────────────────────────────────────────────────────
async function transcribeReel(req, res) {
    ...
    try {
        ...
        const response = { success: true, transcript, reelUrl };
