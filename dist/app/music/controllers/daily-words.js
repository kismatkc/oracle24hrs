...
async function translateToFrench(text) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
        try {
            ...
            if (!res.ok) {
                console.warn(`[daily-words] MyMemory HTTP ${res.status} for: "${text.substring(0, 50)}" (attempt ${attempt + 1})`);
            ...
            console.warn(`[daily-words] MyMemory error for "${text.substring(0, 50)}":`, err.message, `(attempt ${attempt + 1})`);
            if (attempt < MAX_RETRIES) {
                await new Promise((r) => setTimeout(r, 500 * (attempt + 1)));
                continue;
            }
 ...
 * Returns EnglishData with BOTH definition AND example, or null.
 */
async function tryDictionaryApi(word) {
    try {
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 8000);
        const res = await fetch(`${DICT_API}/${encodeURIComponent(word)}`, { signal: controller.signal });
        clearTimeout(timer);
        if (!res.ok)
            return null;
        const data = (await res.json());
...
async function enrichWord(word) {
    ...
    if (!english)
        english = await tryDictionaryApi(word);
    if (!english) {
...
async function ensureDayRollover() {
    ...
    console.log(`[daily-words] Day rollover detected (${stored || "null"} → ${today}) — clearing today_shown`);
 ...
 */
async function selectCandidates(needed, todaySet) {
    ...
    if (cursor < WORDS.length) {
        ...
        while (candidates.length < TARGET && pos < maxScan) {
            ...
            pos++;
            if (todaySet.has(word) || candidateSet.has(word))
                continue;
        ...
        if (pos >= WORDS.length) {
            console.log(`[daily-words] Pool fully scanned (${WORDS.length} words). Future days use LRU recycling.`);
        }
    }
    ...
    if (candidates.length < TARGET) {
        ...
        const lru = await appRedis.zrange(KEY_SHOWN, 0, recycleNeeded - 1);
        for (const w of lru) {
            if (candidates.length >= TARGET)
                break;
            if (todaySet.has(w) || candidateSet.has(w))
                continue;
            candidates.push(w);
            candidateSet.add(w);
        }
 ...
 */
async function getDailyWords(isTestMode) {
    ...
    if (todayWords.length > 0) {
        const entries = await Promise.all(todayWords.map(async (w) => {
            try {
    ...
    if (existingEntries.length >= BATCH_SIZE) {
        console.log(`[daily-words] Returning ${existingEntries.length} cached today-words`);
        return {
    ...
    console.log(`[daily-words] Selected ${newEntries.length} new + ${existingEntries.length} cached = ${allWords.length} words in ${attempts} attempts`);
...
router.get("/daily-words", async (req, res) => {
    try {
        ...
        console.log(`[daily-words] Served ${words.length} words | attempts=${attempts} | cached=${fromCache} | cursor=${cursor}/${WORDS.length}`);
        res.json({
            words,
            meta: {
...
router.post("/daily-words/clear-cache", async (_req, res) => {
    try {
        ...
        for (const pattern of patterns) {
            let scanCursor = "0";
            do {
                const [nextCursor, keys] = await appRedis.scan(scanCursor, "MATCH", pattern, "COUNT", 200);
                scanCursor = nextCursor;
                if (keys.length > 0) {
                    // Strip "app:" prefix since appRedis.del() re-adds it automatically
                    const unprefixed = keys.map((k) => k.startsWith(APP_KEY_PREFIX)
        ...
        console.log(`[daily-words] Cleared ${deletedCount} keys (daily:* + legacy daily-words:*)`);
        res.json({
            ok: true,
...
router.post("/daily-words/preheat", async (req, res) => {
    ...
    (async () => {
        ...
        console.log(`[daily-words] Preheat complete: ${enriched} enriched, ${skipped} skipped`);
    })().catch((err) => console.error("[daily-words] Preheat error:", err.message));
});
