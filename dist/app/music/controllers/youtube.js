...
async function extractStreamUrl(streamingData, yt) {
    if (!streamingData) {
    ...
    console.log(`[youtube] Available formats: ${adaptiveCount} adaptive, ${regularCount} regular`);
    // Try adaptive_formats first (audio-only, better quality)
    let audioFormats = streamingData.adaptive_formats?.filter((f) => f.has_audio && !f.has_video);
    console.log(`[youtube] Found ${audioFormats?.length || 0} audio-only formats`);
    // Fallback to combined formats
    if (!audioFormats || audioFormats.length === 0) {
        audioFormats = streamingData.formats?.filter((f) => f.has_audio);
        console.log(`[youtube] Found ${audioFormats?.length || 0} combined formats with audio`);
    }
    if (!audioFormats || audioFormats.length === 0) {
        // Log what we do have for debugging
        if (streamingData.adaptive_formats?.length) {
            const types = streamingData.adaptive_formats.map((f) => ({
                mime: f.mime_type,
                hasAudio: f.has_audio,
                hasVideo: f.has_video,
            }));
            console.log("[youtube] Adaptive format types:", JSON.stringify(types.slice(0, 5)));
        }
    ...
    const iosCompatibleFormats = audioFormats.filter((f) => {
        const mime = (f.mime_type || "").toLowerCase();
        return IOS_COMPATIBLE_MIMES.some((compatMime) => mime.includes(compatMime.split("/")[1]));
    });
    const webmFormats = audioFormats.filter((f) => {
        const mime = (f.mime_type || "").toLowerCase();
        return mime.includes("webm") || mime.includes("opus");
    });
    console.log(`[youtube] iOS-compatible formats: ${iosCompatibleFormats.length}, WebM/Opus formats: ${webmFormats.length}`);
    // Sort iOS-compatible formats by bitrate (highest first)
    iosCompatibleFormats.sort((a, b) => (b.bitrate || 0) - (a.bitrate || 0));
    ...
    for (let i = 0; i < iosCompatibleFormats.length; i++) {
        ...
        console.log(`[youtube] Trying iOS-compatible format ${i + 1}: ${formatInfo}`);
        try {
            // Method 1: Direct URL
            if (format.url) {
            ...
            if (format.decipher && yt.session?.player) {
                try {
                    ...
                    console.log(`[youtube] Decipher failed for iOS format ${i + 1}:`, e?.message);
                }
            }
            ...
            if (format.signature_cipher && yt.session?.player) {
                try {
                    const url = await format.decipher(yt.session.player);
                    if (url) {
                        console.log("[youtube] Got iOS-compatible stream URL via signature cipher");
                        return { url, format: formatInfo, error: null };
                    }
                }
                catch (e) {
                    console.log(`[youtube] Signature cipher failed for iOS format ${i + 1}:`, e?.message);
                }
            }
        }
        catch (e) {
            console.error(`[youtube] Error with iOS-compatible format ${i + 1}:`, e?.message);
        }
    }
    // If no iOS-compatible formats worked, log warning but DON'T fall back to WebM
    // WebM won't work on iOS anyway
    console.log("[youtube] No iOS-compatible formats available, WebM/Opus won't work on iOS");
    return {
        url: null,
        format: null,
...
async function getStreamUrlWithYtDlp(videoId) {
    return new Promise((resolve) => {
        ...
        console.log("[youtube] yt-dlp fallback: Getting iOS-compatible stream URL for", videoId);
